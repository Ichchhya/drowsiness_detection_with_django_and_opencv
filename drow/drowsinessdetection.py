...
def runmethod():
    ...
    model = k.models.load_model(os.path.join(settings.BASE_DIR, 'face_detector/cnnCat2.h5'))
    path = os.getcwd()
    #cap = cv2.VideoCapture(0)
    cap = cv2.VideoCapture(os.path.join(settings.BASE_DIR,'test.flv'))
    ...
    while (True):
        ...
        if (rpred[0] == 0 and lpred[0] == 0):
            score = score + 1
            cv2.putText(frame, "Closed", (0, height - 20), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
        ...
        if (score > 15):
            ...
            cv2.imwrite(os.path.join(path, 'images.jpg'), frame)
            try:
                playsound('alarm.wav')
